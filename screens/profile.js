          ...
          {userPost.map((item, index) => (
            <View key={index}>
              <TouchableOpacity
                ...
                delayLongPress={200}
                activeOpacity={1}
                onPress={() => navigation.navigate("MyPost", { index })}
              >
                <Image
                  style={{
                    width: imgWidth,
  ...
  React.useEffect(() => {
    ...
    db.collection("follow")
      ...
      .onSnapshot((querySnapshot) => {
        ...
        setFollowing(following.filter((uid) => uid !== auth.currentUser.uid));
      });
    db.collection("follow")
      .doc(auth.currentUser.uid)
    ...
    >
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.top}>
          <Image source={{ uri: user.photoURL }} style={styles.avatart} />
          <View>
            <Text style={{ ...styles.txtTop, color: textColor }}>
              {totalPost.length}
            </Text>
            <Text style={{ ...styles.txtdes, color: textColor }}>Bài viết</Text>
          </View>
          <TouchableOpacity
            onPress={() => {
