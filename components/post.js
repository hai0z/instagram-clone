        ...
        if (doc.exists) {
          ...
          if (like.includes(auth.currentUser.uid)) {
            ...
            postRef
              ...
              .then(() => {
                nofiRef.add({
                  title: `${user.displayName} thích ảnh của bạn`,
                  user,
                ...
                <View style={styles.content}>
                  {item.like.length > 0 ? (
                    <TouchableOpacity
                      activeOpacity={1}
                      onPress={() =>
                        navigation.navigate("Like", { data: item.like })
                      }
                    >
                      <Text style={{ ...styles.txtLike, color: textColor }}>
                  ...
                  {item.caption.trim().length > 0 ? (
                    <View style={{ flexDirection: "row" }}>
                      ...
                      <Text style={{ color: textColor }}> {item.caption}</Text>
                    </View>
                  ) : (
                    <></>
                  )}
